...
int main() {
	...
	is between 0 (48) and 9 (57), because 19 is also a legitimate time. We will add an additional check for hours later. 
	For minutes and seconds it is from 00 to 59, so the first symbol is > 0 and < 6, the second -- > 0 and < 9.
	*/
	
	if ( (((int)hours[0]) < 48 || ((int)hours[0] > 50)) || (((int)hours[1] < 48) || ((int)hours[1] > 57)) )
	{
		...
		if ((stoi(hours) < 0) || (stoi(hours) > 23)) { // here we check that hours are between 0 and 23  
			cout << "Incorrect time (s:hours)" << endl;
		} else {
			cout << "This time is OK!" << endl;
		}
	}
	
	// We check each of strings (hours, minutes and seconds) for correctness, converting it to integer with stoi() function.
